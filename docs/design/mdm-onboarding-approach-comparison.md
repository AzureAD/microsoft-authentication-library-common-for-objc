...
### B1. Completion callback as response object (allowed propagation)

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ NavAction(request.URL = msauth://in_app_enrollement_complete)                │
└──────────────────────────────────────────────────────────────────────────────┘
                │
                │ Allow (do NOT cancel for onboarding work)
                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ Response factory / parser                                                     │
│ - parse msauth://in_app_enrollement_complete into typed response object       │
└──────────────────────────────────────────────────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ Uniform outcome handling pipeline                                              │
│ - completion response object consumed same as other terminal outcomes         │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

### B2. Header-driven ASWebAuth represented as response/operation
...
└──────────────────────────────────────────────────────────────────────────────┘
                │
                │ Evaluate headers for ASWebAuth trigger
                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ Synthesize response object from headers                                        │
│ (e.g., "launch ASWebAuth" response payload with startURL/callback metadata)   │
└──────────────────────────────────────────────────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ Operation executes response: launch ASWebAuthenticationSession                │
└──────────────────────────────────────────────────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ ASWebAuth returns callbackURL                                                  │
└──────────────────────────────────────────────────────────────────────────────┘
                │
                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ Load callbackURL into SAME embedded WKWebView                                 │
│ - webView.load(callbackURL request)                                            │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
```text
For `msauth://enroll` / `msauth://compliance`, actual control still starts in NavAction:

NavAction sees instruction URL
   → must Cancel immediately to prevent wrong navigation
   → run BRT guard once per redirect instruction
   → build nextRequest
   → load nextRequest into same WKWebView

If Approach B is also used, a second response-object path must mirror pieces of this logic,
creating dual-path complexity (boundary interception + response pipeline orchestration).
```
...
- `msauth://enroll` (intercept at NavAction; Cancel → BRT guard → build nextRequest → load in same WKWebView)
- `msauth://compliance` (intercept at NavAction; Cancel → BRT guard → build nextRequest → load in same WKWebView)
